/**
 * Mail sending using nodemailer — loaded lazily so the module is safe to import
 * even when nodemailer is not installed or SMTP is not configured.
 *
 * Fix: removed top-level `import nodemailer` that caused crash when package absent.
 * nodemailer is now require()'d inside sendEmail() only when SMTP is actually configured.
 */
...
export async function sendEmail(payload: MailPayload): Promise<{ sent: boolean; reason?: string }> {
  if (!emailEnabled()) {
    return { sent: false, reason: 'smtp-not-configured' };
  }

  try {
    // Dynamic require — nodemailer absent = graceful no-op, not a build crash
...
export function enquiryReceivedAdminTemplate(data: {
  ...
  const chatSection = data.aiChatSummary
    ? `\n\nAI Chat Summary:\n${data.aiChatSummary}`
    : '';
  ...
  return {
    ...
    text: [
      `New enquiry from ${data.fullName} (${data.email}).`,
      `Travel date: ${when}`,
...
<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:24px;background:#f6f5ef;color:#13261c;">
  ...
  <div style="background:#fff;border-radius:20px;padding:24px;margin-top:16px;border:1px solid #dde4dc;">
    ...
    <table style="width:100%;border-collapse:collapse;">
      <tr><td style="padding:8px 0;font-weight:700;width:38%;">Name</td><td>${data.fullName}</td></tr>
      ...
      <tr><td style="padding:8px 0;font-weight:700;">Trip length</td><td>${data.tripLengthDays != null ? `${data.tripLengthDays} day(s)` : 'Not provided'}</td></tr>
    ...
    ${data.aiChatSummary ? `
    <div style="margin-top:18px;padding:16px;border-radius:16px;background:#f0f7ff;border:1px solid #bfdbfe;">
      <div style="font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#1d4ed8;font-weight:700;">AI Assistant conversation</div>
      <pre style="margin:10px 0 0;font-family:inherit;font-size:0.82rem;white-space:pre-wrap;color:#374151;">${data.aiChatSummary}</pre>
    </div>` : ''}
  </div>
</div>`,
  };
...
export function enquiryReceivedCustomerTemplate(data: { fullName: string }) {
  return {
    ...
    text: `Hi ${data.fullName}, thanks for reaching out to Cape Compass. A consultant will reply shortly.`,
    html: `
<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:24px;background:#f6f5ef;color:#13261c;">
  <div style="background:linear-gradient(135deg,#0d6f54,#c8922d);padding:28px;border-radius:24px;color:#fff;">
    ...
    <p style="margin:0;max-width:420px;line-height:1.6;">Thank you for sharing your travel plans with us. One of our consultants will be in touch soon.</p>
  </div>
  <div style="background:#fff;border-radius:20px;padding:24px;margin-top:16px;border:1px solid #dde4dc;">
...
export function enquiryStatusTemplate(data: {
  fullName: string;
  status: string;
  notes?: string | null;
}) {
