...
export default function ReviewPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews/validate?token=${token}`)
      .then((r) => {
        if (!r.ok) setStatus('invalid');
      })
      .catch(() => setStatus('invalid'));
  }, [token]);

  async function handleSubmit() {
    if (!token || !rating || body.length < 10 || !name) return;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (err.error === 'Review already submitted') {
          setStatus('done');
          return;
        }
      ...
      <section className="section">
        <div className="narrow" style={{ textAlign: 'center', paddingTop: '3rem' }}>
          ...
          <p className="lead" style={{ marginTop: '0.75rem' }}>
            This link is no longer valid. Please contact us if you believe this is an error.
          </p>
        </div>
      </section>
    );
        ...
        <div style={{ maxWidth: 540, margin: '0 auto' }}>
          ...
          <div className="panel" style={{ display: 'grid', gap: '1.5rem' }}>
            ...
            <label>
              ...
              <span style={{ fontSize: '0.72rem', color: 'var(--mist)', textAlign: 'right' }}>
                {body.length} / 1200
              </span>
            </label>
