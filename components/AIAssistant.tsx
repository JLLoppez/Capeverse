...
/**
 * Build a compact plain-text summary of the conversation for pre-filling the enquiry form.
 * Fix bug 18: use the filter index parameter (not indexOf) for reference-safe greeting skip.
 */
function buildChatSummary(messages: Message[]): string {
  return messages
    .filter((_m, i) => i > 0) // skip the opening assistant greeting (always index 0)
    .map((m) => `${m.role === 'user' ? 'Guest' : 'Assistant'}: ${m.content}`)
    .join('\n')
    .slice(0, 1800); // cap to keep URL reasonable
}
...
export function AIAssistant() {
  ...
  function buildEnquiryUrl(): string {
    const summary = buildChatSummary(messages);
    return `/enquiry?${new URLSearchParams({ aiChat: summary }).toString()}`;
  }
  ...
  return (
    <div className="ai-shell premium-ai-shell">
      <div className="ai-header-row">
        ...
        <a
          href={hasRealConversation ? buildEnquiryUrl() : '/enquiry'}
          className="button small outline"
        >
          {hasRealConversation ? 'Send this to a consultant →' : 'Speak to a consultant'}
        </a>
      </div>
      ...
      <div className="chat-window luxury-chat-window">
        ...
        {loading && (
          <div className="chat-bubble assistant">Shaping the best route for you…</div>
        )}
        {error && (
      ...
      {hasRealConversation && (
        <p style={{ fontSize: '0.78rem', color: 'var(--text-faint)', marginTop: '0.5rem', textAlign: 'center' }}>
          When you click &ldquo;Send this to a consultant&rdquo;, your conversation will be attached to the enquiry so the team has full context.
        </p>
      )}
    </div>
  );
